...
To conclude, with SGs there is a need to collect data from several resources during the different stages of an experiment. Also, there is a need to map these data for each user (user’s pre-assessment results, user’s in-game interaction data and user’s post-assessment results) in order to give meaningful insights. Which raises several challenges including user management, data collection and analysis. 
