    ...
    if [[ -e "$exportinProgressFile" ]]; then
            ...
            if [ $ret = 0 ]; then
                ...
                if [[ ${SIMVA_KEYCLOAK_VERSION%%.*} > 18 ]]; then 
                    ...
                    "${SIMVA_HOME}/bin/run-command.sh" /opt/keycloak/bin/kc.sh export --dir "/opt/keycloak/data/export/" --users different_files --users-per-file 100 --realm ${SIMVA_SSO_REALM} --optimized
