...
function __keycloak_login() {
    ...
    if [[ -f $admin_user_file ]]; then 
        ...
        "${SIMVA_HOME}/bin/run-command.sh" /opt/keycloak/bin/kcadm.sh config credentials --server "https://${SIMVA_SSO_HOST_SUBDOMAIN}.${SIMVA_EXTERNAL_DOMAIN}" --realm master --user admin_tmp --password ${SIMVA_KEYCLOAK_ADMIN_PASSWORD}
        ...
        user=$("${SIMVA_HOME}/bin/run-command.sh" /opt/keycloak/bin/kcadm.sh get users -r master -q username=admin_tmp --fields id)
...
function __list_keycloak_resources() {
    ...
    __keycloak_login
    "${SIMVA_HOME}/bin/run-command.sh" /opt/keycloak/bin/kcadm.sh get $keycloak_resource -r ${SIMVA_SSO_REALM} --fields $fields
}
...
function __get_keycloak_resource() {
    ...
    __keycloak_login
    "${SIMVA_HOME}/bin/run-command.sh" /opt/keycloak/bin/kcadm.sh get -r ${SIMVA_SSO_REALM} "$keycloak_resource/${id}"
}
...
function __add_keycloak_resource() {
    ...
    shift 1
    __keycloak_login
    "${SIMVA_HOME}/bin/run-command.sh" /opt/keycloak/bin/kcadm.sh create $keycloak_resource -r ${SIMVA_SSO_REALM} -f $keycloak_resource_file -i
}
...
function __update_keycloak_resource() {
    ...
    shift 1
    __keycloak_login
    "${SIMVA_HOME}/bin/run-command.sh" /opt/keycloak/bin/kcadm.sh update $keycloak_resource/$endpointId -r ${SIMVA_SSO_REALM} -f $keycloak_resource_file
}
...
function __update_realm() {
    ...
    __keycloak_login
    "${SIMVA_HOME}/bin/run-command.sh" /opt/keycloak/bin/kcadm.sh update "realms/$SIMVA_SSO_REALM" -f $keycloak_resource_file
}
