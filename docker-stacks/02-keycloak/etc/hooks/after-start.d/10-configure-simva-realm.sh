    ...
    if [[ ! -e "${SIMVA_PROJECT_DIR}/.simva-initialized" ]]; then 
        ...
        if [[ -e "${SIMVA_CONFIG_HOME:-/home/vagrant/docker-stacks/config}/keycloak/simva-realm-export/${SIMVA_SSO_REALM:-simva}-realm-full.json" ]]; then    
            ...
            if [[ -e "$migrationinProgressFile" ]]; then
                ...
                docker exec 02-keycloak-keycloak-1 /opt/keycloak/bin/kc.sh import --file "/opt/keycloak/data/export/${SIMVA_SSO_REALM:-simva}-realm-full.json" --override true --optimized
