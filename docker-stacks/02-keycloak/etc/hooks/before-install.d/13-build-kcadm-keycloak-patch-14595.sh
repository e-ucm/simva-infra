...
if [[ -e "${keycloak_dir}/integration/client-cli/admin-cli/pom.xml" ]]; then
    mv "${keycloak_dir}/integration/client-cli/admin-cli/pom.xml" "${keycloak_dir}/integration/client-cli/admin-cli/pom.xml.template" 
    cat "${keycloak_dir}/integration/client-cli/admin-cli/pom.xml.template" \
       | sed -e '/<artifactId>httpclient<\/artifactId>/ a\            <version>4.5.12</version>' \
      > "${keycloak_dir}/integration/client-cli/admin-cli/pom.xml"
fi
#        patch -p1 <<PATCH
