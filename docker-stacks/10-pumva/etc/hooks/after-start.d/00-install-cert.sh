...
if [[ -f "$SIMVA_DATA_HOME/pumva/sqlite_initialisation_in_progress" ]]; then
    echo "Migration in progress. Skipping certificate installation."
    exit 0
fi
...
container_list=("pumva");
for container_name in "${container_list[@]}"; do
    export RUN_IN_CONTAINER_NAME="$container_name"
    # Get the internal CA certificate content from inside the container
    set +e
    internal_ca=$("${SIMVA_BIN_HOME}/run-command.sh" /bin/sh -c "cat \"/usr/local/share/ca-certificates/internal-CA.crt\" 2>/dev/null")
    res=$?
    set -e
    if [[ ! "$res" == "0" ]]; then
    ...
    if [[ $install == "true" ]]; then
        ...
        "${SIMVA_BIN_HOME}/run-command.sh" /bin/sh -c "cp /var/lib/pumva/tls/ca/$SIMVA_ROOT_CA_FILENAME \"/usr/local/share/ca-certificates/internal-CA.crt\";
