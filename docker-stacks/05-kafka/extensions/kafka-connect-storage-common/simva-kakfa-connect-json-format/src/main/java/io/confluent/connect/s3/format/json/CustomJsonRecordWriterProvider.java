...
//import io.confluent.connect.s3.format.RecordViews.RecordView;
...
public class CustomJsonRecordWriterProvider extends RecordViewSetter
    implements RecordWriterProvider<S3SinkConnectorConfig> {

  private static final Logger log = LoggerFactory.getLogger(CustomJsonRecordWriterProvider.class);
  ...
  CustomJsonRecordWriterProvider(S3Storage storage, JsonConverter converter) {
  ...
  public String addKakfaVersionAndTimeStampToFileName(String filename, String initialExtension) {
    if (filename.endsWith(initialExtension)) {
      int index = filename.lastIndexOf(initialExtension);
      filename=filename.substring(0, index);
    }
    // ✅ Fetch Kafka Version from Environment Variable
    final String kafkaVersion = System.getenv("KAFKA_VERSION") != null
            ? System.getenv("KAFKA_VERSION")
    ...
    final String customFilename = String.format(
        "%s_kafka_v%s_%s", filename, kafkaVersion, timestamp
    );
  ...
  public RecordWriter getRecordWriter(final S3SinkConnectorConfig conf, final String filename) {
    try {
      log.info("Sink record with view {}", filename);
      return new S3RetriableRecordWriter(
          new IORecordWriter() {
            final String adjustedFilename = getAdjustedFilename(recordView, addKakfaVersionAndTimeStampToFileName(filename,getExtension()),
                getExtension());
            ...
            public void write(SinkRecord record) throws IOException {
              log.info("Sink record with view {}: {}", recordView,
