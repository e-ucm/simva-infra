...
if [[ -e "${SIMVA_CA_FILE}" ]]; then
  ...
  if [[ ${ca_already_imported} -ne 0 ]]; then
    ...
    launch_bash_options=$-
    set +e
    if [[ "${KAFKA_VERSION%%.*}" -ge 7 ]]; then 
      keytool -importcert -trustcacerts -noprompt -cacerts -storepass "${JDK_TRUSTORE_PASSWORD}" -file "${SIMVA_CA_FILE}" -alias "${SIMVA_CA_ALIAS}" >/dev/null 2>&1;
    else
