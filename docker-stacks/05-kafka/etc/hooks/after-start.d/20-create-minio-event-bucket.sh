...
if [ ! -e "${SIMVA_DATA_HOME}/minio/minio-events-initialized" ]; then
        ...
        echo $info
        echo $arn
        docker compose exec minio-client /bin/sh -c "/usr/bin/mc --debug event add --event put --suffix \"${SIMVA_SINK_TOPICS_DIR}/${SIMVA_TRACES_TOPIC}/_id=*/traces+*.json\" simva-minio/${SIMVA_TRACES_BUCKET_NAME} $arn"
