...
if [[ $SIMVA_SHLINK_USE_SIMVA_EXTERNAL_DOMAIN  == "false" ]]; then 
    if [[ ! -e "${SIMVA_TRAEFIK_SHLINK_FULLCHAIN_CERT_FILE}" ]]; then
        if [[ "${SIMVA_TLS_GENERATE_SELF_SIGNED}" == "true" ]]; then
            ...
            echo "Please insert your shlink fullchain certificate at '${SIMVA_TRAEFIK_SHLINK_FULLCHAIN_CERT_FILE}' or run using SIMVA_TLS_GENERATE_SELF_SIGNED=true to self generate your certificates."
            exit 1;
        fi
    fi
fi