...
backup_data $SIMVA_TLS_HOME $BACKUP_TLS_DIR $(cat "${SIMVA_BACKUP_HOME}/${STACK_NAME}/.timestamp") true
echo "✅ Backup completed: $BACKUP_TLS_DIR"

BACKUP_ENV_DIR="${SIMVA_BACKUP_HOME}/${STACK_NAME}/simva-env"
backup_data "$SIMVA_ETC_HOME/simva.d" $BACKUP_ENV_DIR $(cat "${SIMVA_BACKUP_HOME}/${STACK_NAME}/.timestamp") true
