...
if [[ -d "${SIMVA_DATA_HOME}/shlink/data" ]]; then 
  "${SIMVA_HOME}/bin/volumectl.sh" exec "shlink_db" "/shlink_db" "
  ...
  "
  ${SIMVA_HOME}/bin/purge-folder-contents.sh "${SIMVA_DATA_HOME}/shlink"
fi

if [[ -d "${SIMVA_DATA_HOME}/shlink/config" ]]; then 
  "${SIMVA_HOME}/bin/volumectl.sh" exec "shlink_config" "/shlink_config" "
    # Set ownership recursively
  ...
  "
  ${SIMVA_HOME}/bin/purge-folder-contents.sh "${SIMVA_DATA_HOME}/shlink/config"
fi