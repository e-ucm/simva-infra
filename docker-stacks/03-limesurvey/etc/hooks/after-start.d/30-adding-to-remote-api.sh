...
if [[ ! -e "${SIMVA_DATA_HOME}/limesurvey/.initializedRemoteControl" ]]; then 
    ...
    "${SIMVA_BIN_HOME}/run-command.sh" bash -c  "sed -i '\$s/^[[:space:]]*}//; \$d' $REMOTE_FILE_PATH"
    ...
    "${SIMVA_BIN_HOME}/run-command.sh" bash -c "cat /tmp/$NEW_FUNCTION_FILE_NAME >> $REMOTE_FILE_PATH"

    # Step 4: Add the closing brace back at the end of the file
    "${SIMVA_BIN_HOME}/run-command.sh" bash -c "echo '}' >> $REMOTE_FILE_PATH"
