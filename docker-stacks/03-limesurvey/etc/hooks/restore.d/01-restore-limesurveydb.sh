...
_stop_docker_container_if_running
"${SIMVA_HOME}/bin/volumectl.sh" copylv $BACKUP_DIR "ls_maria_db_backup_data" $BACKUP_FILE_COMPRESSED "restore" true
"${SIMVA_HOME}/bin/volumectl.sh" exec "ls_maria_db_backup_data" "/dump" "ls -lia /dump/restore"
...
"${SIMVA_HOME}/bin/run-command.sh" bash -c "mysql -h'mariadb.${SIMVA_LIMESURVEY_HOST_SUBDOMAIN}.${SIMVA_INTERNAL_DOMAIN}' -uroot -p'${SIMVA_LIMESURVEY_MYSQL_ROOT_PASSWORD}' < '/dump/restore/$BACKUP_FILE'"
