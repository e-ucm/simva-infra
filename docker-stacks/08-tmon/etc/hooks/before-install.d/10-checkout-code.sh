...
SIMVA_TMON_GIT_REF=${SIMVA_TMON_GIT_REF:-plotly-dash}
...
if [[ ${RUNCHECKOUTCODE} == true ]] ; then
    ...
    mkdir -p ${SIMVA_DATA_HOME}/tmon
    
    # Checkout code in temp dir
    tmp_dir=$(mktemp -d)
    git clone --depth 1 --branch ${SIMVA_TMON_GIT_REF} ${SIMVA_TMON_GIT_REPO_URL} ${tmp_dir} > /dev/null 2>&1;
    rsync -avh --delete --itemize-changes ${tmp_dir}/ ${SIMVA_DATA_HOME}/tmon/ > /dev/null 2>&1
    sudo chmod -R 777 ${SIMVA_DATA_HOME}/tmon/
fi